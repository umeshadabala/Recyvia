...
export interface UserProfile {
  ...
  name: string;
  email?: string;
  role: 'individual' | 'business' | 'collector' | 'recycler';
  address?: string;
  pincode?: string;
  createdAt: string;
  updatedAt: string;
...
export const BackendAuthService = {
  ...
  verifyOtp: async (
    ...
    inputOtp: string,
    name?: string,
    role: 'individual' | 'business' | 'collector' | 'recycler' = 'individual'
  ): Promise<{ success: boolean; message: string; session?: AuthSession; cookieHeader?: string }> => {
    const cleanPhone = phone.trim().replace(/\s+/g, '');
    const otpRecord = await DynamoRepository.getItem(`OTP#${cleanPhone}`, 'META');
    ...
    let userRecord = await DynamoRepository.getItem(`USER#${cleanPhone}`, 'PROFILE');
  ...
  getSession: async (token: string): Promise<AuthSession | null> => {
    ...
    const item = await DynamoRepository.getItem(`SESSION#${token}`, 'META');
    if (!item) return null;
  ...
  // Clear session
  logout: async (token: string): Promise<{ success: boolean; cookieHeader: string }> => {

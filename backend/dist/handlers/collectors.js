...
const handler = async (event) => {
    try {
        if (event.httpMethod === 'GET') {
            const items = await dynamoRepository_1.DynamoRepository.scanByEntityPrefix('COLLECTOR#');
            const collectors = items.map(({ pk, sk, ...data }) => data);
            return (0, responses_1.successResponse)(collectors);
        }

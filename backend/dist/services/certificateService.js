...
exports.BackendCertificateService = void 0;
const dynamoRepository_1 = require("../repositories/dynamoRepository");
exports.BackendCertificateService = {
    createCertificateForPickup: async (pickup, transactionId) => {
    ...
    getAllCertificates: async () => {
        const items = await dynamoRepository_1.DynamoRepository.scanByEntityPrefix('CERTIFICATE#');
        return items
            .filter((i) => i.sk === 'META')
            .map(({ pk, sk, ...data }) => data);
    },
    getCertificateById: async (certId) => {
        const item = await dynamoRepository_1.DynamoRepository.getItem(`CERTIFICATE#${certId}`, 'META');
        if (!item)
            return null;
